...
class Configsetup
{
    ...
    protected function seed_knowledge()
    {
        ...
        $this->upsert_seed('core_knowledge_articles', 'code', 'ventas_pedido_entrega_inventario', [
            ...
            'content' => '<h3>Objetivo</h3><p>Separar documentos para que cada modulo haga una sola cosa: la cotizacion negocia, el pedido confirma, la entrega mueve inventario y la factura timbra fiscalmente.</p><h4>Flujo base</h4><ol><li>Crear cotizacion en <strong>Ventas</strong>.</li><li>Al aprobarla, abrir detalle y usar <strong>Crear pedido</strong>.</li><li>Desde el pedido usar <strong>Crear entrega</strong>. La entrega descuenta existencia del almacen general y deja movimiento en Inventario.</li><li>Desde la entrega usar <strong>Facturar entrega</strong>. Facturacion copia las partidas y prepara el CFDI.</li></ol><h4>Inventario</h4><ul><li>La salida de almacen se registra con <code>core_inventory_movements</code> y actualiza saldos en <code>core_inventory_stock_balances</code>.</li><li>El almacen inicial es <strong>GENERAL</strong>; los saldos se muestran por producto y almacen.</li><li>Admin &gt; Inventario concentra existencias, entregas, movimientos y auditoria de stock.</li><li>Los movimientos manuales funcionan como documentos con multiples partidas para entradas, salidas, mermas y traspasos por lote.</li><li>La auditoria compara existencia por almacen contra suma de movimientos para detectar diferencias por carga inicial o ajustes pendientes.</li><li>La factura directa no descuenta inventario; debe existir entrega relacionada para controlar salida fisica.</li></ul><h4>Siguiente crecimiento</h4><ul><li>Reservar inventario desde pedido y liberar reserva si se cancela.</li><li>Recibir inventario desde compras autorizadas.</li><li>Relacionar entregas con paqueterias, documentos y evidencias.</li><li>Agregar documentos completos de inventario con folio propio y autorizaciones por monto/departamento.</li></ul>',
            'sort_order' => 61,
            'active' => 1,
            'created_at' => time(),
            'updated_at' => time(),
