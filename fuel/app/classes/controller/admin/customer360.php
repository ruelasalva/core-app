...
class Controller_Admin_Customer360 extends Controller_Adminbase
{
    public function before()
    {
        ...
        if (!$this->can_access_customer360()) {
    ...
    {
        $this->template->title = 'Vista 360 de Cliente';
        $this->template->content = \View::forge('admin/customer360/index', array(
            'can_crm' => $this->is_super_admin || \Auth::has_access('crm.access[view]'),
    ...
    public function action_data()
    {
        ...
        if ($party_id <= 0) {
            return $this->json(false, 'Captura un cliente valido.', array(), array('party_id_required'), 400);
        }

        try {
            $service = new Service_Core_EntityHub_Customer360();
            $data = $service->data($party_id, $this->user_id);

            if (empty($data['found'])) {
                return $this->json(false, 'Cliente no encontrado.', array(), array('customer_not_found'), 404);
            }
            if (empty($data['visible'])) {
                return $this->json(false, 'No tienes permiso para ver este cliente.', array(), array('permission_denied'), 403);
            }

            return $this->json(true, 'Vista 360 de Cliente consultada correctamente.', $data);
        } catch (\Exception $e) {
            \Log::error('Customer360 data error: '.$e->getMessage());
            return $this->json(false, 'No fue posible cargar Vista 360 de Cliente.', array(), array('internal_error'), 500);
        }
    }
    ...
    public function action_search()
    {
        ...
        if (!\DBUtil::table_exists('core_parties')) {
            return $this->json(false, 'Falta la tabla de clientes.', array(), array('core_parties_missing'), 500);
        }
        ...
        try {
            ...
            $visibility->apply_customer_scope($query, (int) $this->user_id, 'p');
            ...
            return $this->json(true, 'Clientes consultados correctamente.', array('customers' => $items));
        } catch (\Exception $e) {
            \Log::error('Customer360 search error: '.$e->getMessage());
            return $this->json(false, 'No fue posible buscar clientes.', array(), array('internal_error'), 500);
        }
    }

    protected function can_access_customer360()
    {
        if ($this->is_super_admin) {
            return true;
        }

        foreach (array('customers.access[view]', 'crm.access[view]', 'business.access[view]', 'parties.access[view]') as $permission) {
    ...
    protected function json_guard()
    {
        ...
        if (!$this->can_access_customer360()) {
            return $this->json(false, 'No tienes permiso para consultar Vista 360 de Cliente.', array(), array('permission_denied'), 403);
        }
