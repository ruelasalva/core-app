...
class Configsetup
{
    ...
    protected function seed_knowledge()
    {
        ...
        $this->upsert_seed('core_knowledge_articles', 'code', 'compras_xml_sat_inventario', [
            ...
            'content' => '<h3>Objetivo</h3><p>Permitir que una factura XML de proveedor alimente Compras sin contaminar el catalogo interno. El proveedor puede facturar una descripcion distinta a la que Core-App vende; por eso cada concepto se revisa antes de crear la compra y mover inventario.</p><h4>Flujo desde Auditoria SAT</h4><ol><li>Entra a <strong>Admin &gt; Auditoria SAT</strong>.</li><li>Importa el XML o selecciona un CFDI recibido ya cargado.</li><li>Abre el detalle y presiona <strong>Convertir a compra</strong>.</li><li>Por cada partida elige <strong>Compra interna</strong>, <strong>Servicio</strong> o <strong>Producto para venta</strong>.</li><li>Si es producto para venta, selecciona el SKU interno y almacen. Si no existe, marca <strong>Crear pendiente</strong> y captura SKU/nombre interno.</li><li>Activa <strong>Guardar equivalencia</strong> para que Core-App recuerde que esa clave o descripcion del proveedor corresponde al SKU interno elegido.</li><li>Usa <strong>Guardar equivalencias</strong> si solo quieres preparar el mapeo sin crear todavia la compra, o <strong>Crear compra</strong> para generar orden, factura de proveedor y entrada de inventario.</li></ol><h4>Reglas operativas</h4><ul><li>Servicios y compras internas no afectan inventario.</li><li>Los productos creados desde XML quedan no publicados y sin precio de venta para que Comercial los revise antes de mostrarlos en frontend.</li><li>La equivalencia reutilizable por proveedor vive en <code>core_purchase_supplier_product_mappings</code>; el mapeo de una compra ya convertida vive en <code>core_purchase_cfdi_line_mappings</code>.</li><li>Si el proveedor vuelve a mandar la misma clave o descripcion, el modal sugiere el SKU interno y muestra la etiqueta <strong>Sugerida</strong>.</li><li>La entrada de almacen se registra en <code>core_inventory_movements</code> con tipo <code>purchase_in</code> y actualiza <code>core_inventory_stock_balances</code>.</li><li>Compras no debe guardar XML/PDF como campos sueltos; los archivos siguen perteneciendo a SAT o Documentos.</li></ul><h4>Validacion</h4><ol><li>Despues de guardar equivalencias, vuelve a abrir el CFDI y confirma que la partida salga como sugerida.</li><li>Despues de convertir, revisa <strong>Admin &gt; Compras</strong> para ver la orden y factura.</li><li>Revisa <strong>Admin &gt; Inventario</strong> para confirmar las entradas de almacen.</li><li>Revisa <strong>Admin &gt; Comercial</strong> si se crearon productos pendientes desde XML.</li></ol>',
            'sort_order' => 56,
            'active' => 1,
            'created_at' => time(),
            'updated_at' => time(),
