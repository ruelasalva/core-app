...
class Controller_Admin_Sales extends Controller_Adminbase
{
    ...
    public function action_data()
    {
        try {
            ...
            return $this->json_response($this->sales_read_model()->dashboard_data($filters));
        } catch (\Exception $e) {
    ...
    protected function sales_read_model()
    {
        if (!$this->sales_read_model) {
            $this->sales_read_model = Service_Core_Sales_ReadModel::forge([
                'user_id' => (int) $this->user_id,
                'user_group' => (int) $this->user_group,
                'is_super_admin' => (bool) $this->is_super_admin,
                'department_id' => (int) $this->employee_department_id(),
            ]);
        }
    ...
    {
        return $this->sales_read_model()->quotes($filters ?: $this->period_filters());
    }

    protected function quote_orders($quote_id)
    {
        return $this->sales_read_model()->quote_orders($quote_id);
    }

    protected function order_deliveries($order_id)
    {
        return $this->sales_read_model()->order_deliveries($order_id);
    }

    protected function order_items($order_id)
    {
        return $this->sales_read_model()->order_items($order_id);
    }
    ...
    {
        return $this->sales_read_model()->quote_items($quote_id);
    }
    ...
    {
        return $this->sales_read_model()->stats($filters ?: $this->period_filters());
    }

    protected function orders(array $filters = [])
    {
        return $this->sales_read_model()->orders($filters ?: $this->period_filters());
    }

    protected function deliveries(array $filters = [])
    {
        return $this->sales_read_model()->deliveries($filters ?: $this->period_filters());
    }
    ...
    {
        return $this->sales_read_model()->options();
    }
    ...
    {
        return $this->sales_read_model()->product_options($filters);
    }

    protected function select_rows($table, $value_field, $label_field, array $where = [])
    {
        return $this->sales_read_model()->select_rows($table, $value_field, $label_field, $where);
    }
    ...
    {
        return $this->sales_read_model()->product_price_ranges($product_id);
    }

    protected function media_url($path)
    {
        return $this->sales_read_model()->media_url($path);
    }
