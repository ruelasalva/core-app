...
class Service_Core_Workspace_Widgets_Erp_Pendingquotes extends \Service_Core_Workspace_Widgets_Erp_Base
{
    public static function manifest()
    {
        return [
            ...
            'description' => 'Últimas cotizaciones pendientes o solicitadas.',
            'category' => 'commercial',
            'type' => 'list',
            'icon' => 'bi bi-receipt',
            'color' => 'primary',
    ...
    {
        $table = 'core_sales_quotes';
        if (!$this->table_exists($table)) {
            return $this->payload([]);
        }

        $folio = $this->field($table, ['folio', 'quote_number', 'code']);
        $party = $this->field($table, ['party_id', 'customer_id']);
        $total = $this->field($table, ['total', 'grand_total', 'amount_total']);
        $status = $this->field($table, ['status']);
        $date = $this->field($table, ['created_at', 'quote_date', 'date']);
        ...
        try {
            ...
            $this->safe_query_error('pending_quotes', $e);
            return $this->payload([]);
        }
        ...
        foreach ($rows as $row) {
            ...
            if (!in_array($status_value, ['pending', 'requested'], true)) {
                continue;
            }
    ...
    protected function payload(array $rows)
    {
        return $this->compact_table_payload(
            ...
            [
                'icon' => 'bi bi-receipt',
                'title' => 'Sin cotizaciones pendientes.',
                'message' => 'No hay cotizaciones solicitadas o pendientes para mostrar.',
            ],
            [
                'label' => 'Abrir Ventas',
                'url' => \Uri::create('admin/sales?view=quotes'),
