<div id="app-purchases">
    <div class="row">
        ...
        <div class="col-lg-3"><div class="small-box bg-warning"><div class="inner"><h3>{{ stats.open_orders || 0 }}</h3><p>Ordenes abiertas</p></div><div class="icon"><i class="bi bi-hourglass-split"></i></div></div></div>
        ...
        <div class="card-body">
            ...
            <div v-show="!loading && tab === 'orders'">
                ...
                <table class="table table-bordered table-hover">
                    <thead><tr><th>Folio</th><th>Proveedor</th><th>Estado</th><th>Subtotal</th><th>IVA</th><th>Retenciones</th><th>Total</th><th>Fecha</th><th>Acciones</th></tr></thead>
                    <tbody>
                        <tr v-for="order in orders" :key="order.id">
                            ...
                            <td>{{ order.currency_code }} {{ money(order.subtotal) }}</td>
                            <td>{{ order.currency_code }} {{ money(order.tax_total) }}</td>
                            <td>{{ order.currency_code }} {{ money(order.retention_total) }}</td>
                        ...
                        <tr v-if="orders.length === 0"><td colspan="9" class="text-center text-muted">Sin ordenes.</td></tr>
                    </tbody>
                </table>
            </div>
            ...
            <div v-show="!loading && tab === 'invoices'">
                ...
                <table class="table table-bordered table-hover">
                    <thead><tr><th>Folio</th><th>Proveedor</th><th>OC</th><th>UUID</th><th>Estado</th><th>Total</th><th>Acciones</th></tr></thead>
                    <tbody>
            ...
            <div v-show="!loading && tab === 'receipts'">
                ...
                <table class="table table-bordered table-hover">
                    <thead><tr><th>Folio</th><th>Proveedor</th><th>Estado</th><th>Pago programado</th><th>Total</th><th>Facturas</th></tr></thead>
                    <tbody>
                        ...
                        <tr v-if="receipts.length === 0"><td colspan="6" class="text-center text-muted">Sin contrarecibos.</td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal-dialog modal-xl"><div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-2"><label>Estado</label><select class="form-control" v-model="orderForm.status"><option value="draft">Borrador</option><option value="authorized">Autorizada</option><option value="partial">Parcial</option><option value="closed">Cerrada</option><option value="cancelled">Cancelada</option></select></div>
                </div>
    ...
    <div class="modal fade" id="modal-receipt" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg"><div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row mt-2"><div class="col-md-6"><label>Fecha</label><input type="date" class="form-control" v-model="receiptForm.issue_date"></div><div class="col-md-6"><label>Pago programado</label><input type="date" class="form-control" v-model="receiptForm.scheduled_payment_date"></div></div>
...
window.onload = function() {
    new Vue({
        ...
        data: {
            ...
            options: { suppliers: [], departments: [], payment_terms: [], currencies: [], taxes: [], retentions: [] },
            stats: {},
            orderForm: { items: [] },
            invoiceForm: {},
        ...
        computed: {
            pendingInvoices: function() {
                ...
                return this.invoices.filter(function(i) { return String(i.party_id) === party && i.status !== 'paid' && Number(i.balance_due) > 0; });
            }
        },
        ...
        methods: {
            ...
            newOrder: function(){ this.orderForm = { party_id: '', order_date: this.today(), expected_date: '', currency_code: 'MXN', status: 'draft', notes: '', internal_notes: '', items: [] }; this.resetLine(); this.showModal('modal-order'); },
            ...
            newReceipt: function(){ this.receiptForm = { party_id: '', invoice_ids: [], issue_date: this.today(), scheduled_payment_date: '', currency_code: 'MXN', status: 'draft', notes: '' }; this.showModal('modal-receipt'); },
            ...
            post: function(url, payload, modal){ var self = this; fetch(url, window.coreAppFetchOptions(payload)).then(function(r){ return r.json(); }).then(function(data){ if (data.error) { self.error = data.error; return; } self.hideModal(modal); self.load(); }); },
